...
RSpec.describe Dependabot::PreCommit::UpdateChecker::LatestVersionFinder do
  ...
  describe "cooldown filtering" do
    ...
    let(:github_client) { fake_github_client }
    let(:github_releases) { {} }
    ...
    context "when latest version is in cooldown" do
      ...
      it "falls back to a previous version not in cooldown" do
        stub_cooldown_candidate_checks(
          ...
          git_dates: {
            "latest_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
      ...
      it "uses a newer GitHub release published_at over an older git date" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag, older_tag],
          git_dates: {
      ...
      it "uses a newer git date over an older GitHub release published_at" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag, older_tag],
          git_dates: {
            "latest_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
      ...
      it "falls back to git metadata when GitHub has no release for the tag" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag],
          git_dates: { "latest_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z") }
        )
        stub_github_release_lookup({})
      ...
      it "uses git tag creatordate when GitHub has no release for an annotated tag" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag, older_tag],
          git_dates: {
            "v6.0.0" => recent_date.iso8601,
      ...
      it "fails closed for only the tag whose GitHub release lookup errors" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag, older_tag],
          git_dates: {
            "latest_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z"),
        ...
        allow(Dependabot::Clients::GithubWithRetries)
          ...
          .and_return(github_client)
        allow(github_client).to receive(:releases).and_return([])
        allow(github_client).to receive(:release_for_tag).with("pre-commit/pre-commit-hooks", "v6.0.0")
                                                         .and_raise(StandardError, "api down")
        allow(github_client).to receive(:release_for_tag).with("pre-commit/pre-commit-hooks", "v5.0.0")
      ...
      it "fails closed when GitHub client setup fails" do
        stub_cooldown_candidate_checks(
          candidates: [latest_tag],
          git_dates: { "latest_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z") }
        )
        allow(Dependabot::Clients::GithubWithRetries)
          .to receive(:for_source)
          .and_raise(StandardError, "setup failed")

        result = finder.latest_release_version
        expect(result.to_s).to eq("4.4.0")
      end

      it "normalizes refs/tags prefixes when looking up GitHub releases" do
        prefixed_latest_tag = latest_tag.merge(tag: "refs/tags/v6.0.0")
        prefixed_older_tag = older_tag.merge(tag: "tags/v5.0.0")
        stub_cooldown_candidate_checks(
          candidates: [prefixed_latest_tag, prefixed_older_tag],
          git_dates: {
            "latest_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z"),
            "older_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z")
          }
      ...
      context "with a prefixed current version ref" do
        let(:dependency) do
          Dependabot::Dependency.new(
            name: "https://github.com/#{dependency_name}",
            version: "tags/v4.4.0",
        ...
        it "does not select versions older than the current pinned ref" do
          prefixed_latest_tag = latest_tag.merge(tag: "tags/v6.0.0")
          prefixed_lower_tag = {
            tag: "tags/v3.0.0",
            version: Dependabot::PreCommit::Version.new("3.0.0"),
          ...
          stub_cooldown_candidate_checks(
            candidates: [prefixed_latest_tag, prefixed_lower_tag],
            git_dates: {
              "latest_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
              "lower_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z")
            }
          )
          stub_github_release_lookup({})
        ...
        context "when the current ref is a prerelease" do
          let(:dependency) do
            Dependabot::Dependency.new(
              name: "https://github.com/#{dependency_name}",
              version: "tags/v2.0.0-beta",
              requirements: [{
                requirement: nil,
                groups: [],
                file: ".pre-commit-config.yaml",
          ...
          it "does not select versions older than the current prerelease ref" do
            release_tag = {
              tag: "tags/v2.0.0",
              version: Dependabot::PreCommit::Version.new("2.0.0"),
              commit_sha: "release_sha"
            }
            lower_tag = {
              tag: "tags/v1.9.0",
              version: Dependabot::PreCommit::Version.new("1.9.0"),
              commit_sha: "lower_sha"
            }

            stub_cooldown_candidate_checks(
              candidates: [release_tag, lower_tag],
              git_dates: {
                "release_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
                "lower_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z")
              }
            )
            stub_github_release_lookup({})
            ...
            result = finder.latest_release_version
            expect(result.to_s).to eq("2.0.0.pre.beta")
          end
        end
      end

      context "when pinned to a commit SHA with a known current tag" do
        let(:reference) { "6f6a02c2c85a1b45e39c1aa5e6cc40f7a3d6df5e" }
        let(:dependency) do
          Dependabot::Dependency.new(
            name: "https://github.com/#{dependency_name}",
        ...
        it "does not fall back to tags lower than the current tag" do
          lower_tag = {
            tag: "v3.0.0",
            version: Dependabot::PreCommit::Version.new("3.0.0"),
            commit_sha: "lower_sha"
          ...
          allow_any_instance_of(Dependabot::GitCommitChecker) # rubocop:disable RSpec/AnyInstance
            .to receive(:local_tag_for_pinned_sha)
            .and_return("v4.4.0")
          stub_cooldown_candidate_checks(
            candidates: [latest_tag, lower_tag],
            git_dates: {
              "latest_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
              "lower_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z")
            }
        ...
        it "considers allowed version tags instead of matching raw SHA precision" do
          middle_tag = {
            tag: "v5.0.0",
            version: Dependabot::PreCommit::Version.new("5.0.0"),
            commit_sha: "middle_sha"
          }
          lower_tag = {
            tag: "v3.0.0",
            version: Dependabot::PreCommit::Version.new("3.0.0"),
          ...
          allow_any_instance_of(Dependabot::GitCommitChecker) # rubocop:disable RSpec/AnyInstance
            ...
            .and_return("v4.4.0")
          allow_any_instance_of(Dependabot::GitCommitChecker) # rubocop:disable RSpec/AnyInstance
            .to receive(:pinned_ref_looks_like_commit_sha?)
            .and_return(true)
          allow_any_instance_of(Dependabot::GitCommitChecker) # rubocop:disable RSpec/AnyInstance
            .to receive(:local_tags_for_allowed_versions)
            .and_return([latest_tag, middle_tag, lower_tag])
          allow_any_instance_of(Dependabot::GitCommitChecker) # rubocop:disable RSpec/AnyInstance
            .to receive(:dependency_source_details)
            .and_return({ type: "git", url: "https://github.com/pre-commit/pre-commit-hooks",
                          ref: reference, branch: nil })
          stub_cooldown_git_dates(
            "latest_sha" => recent_date.strftime("%Y-%m-%d %H:%M:%S %z"),
            "middle_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z"),
            "lower_sha" => old_date.strftime("%Y-%m-%d %H:%M:%S %z")
          )
          stub_github_release_lookup({})
      ...
      context "when pinned to a commit SHA with a frozen version comment" do
        ...
        it "does not fall back to tags lower than the comment version" do
          lower_tag = {
            tag: "v3.0.0",
            version: Dependabot::PreCommit::Version.new("3.0.0"),
            commit_sha: "lower_sha"
